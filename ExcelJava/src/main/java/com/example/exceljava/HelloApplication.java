...
public class HelloApplication extends Application {
    ...
    public void start(Stage stage) throws IOException {
        ...
        Button button = new Button("Загрузить xlsx файл");
    ...
    private void parseExcel(File file) {
        ...
        try (Workbook workbook = new XSSFWorkbook(file)) {
            ...
            for (Row row : sheet) {
                ...
                Cell dateCell = row.getCell(5); // колонка Ф(дата)
                ...
                double total = row.getCell(4).getNumericCellValue(); // колонка Е(итоговая цена)
    ...
    private void charUpd(){
        ...
        Map<Integer, Double> profitByMonth = monthlyProfits.stream()
                ...
                .collect(Collectors.groupingBy(
                        MonthlyProfit::getMonth,
                        Collectors.summingDouble(MonthlyProfit::getProfit)
                ));
